...
To take control of the `IT-COMPUTER3$` machine account, we can exploit a Time Roasting attack. This attack sends a malformed NTP (Network Time Protocol) request to the DC on behalf of the computer account. The server responds with a timestamp encrypted using the account's NTLM hash, which can be cracked offline.
...
Once the password for `IT-COMPUTER3$` is cracked, we can abuse its `AddSelf` privilege to add the machine to the `HelpDesk` group:
...
Next, leveraging the HelpDesk group permissions, we reset the password for the user `BB.MORGAN`:
...
To bypass this restriction, we use `IT-COMPUTER3$` to remove `BB.MORGAN` from the group:
...
We can now successfully request the TGT and access the system.
